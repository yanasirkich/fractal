...
/*   Updated: 2024/11/04 04:18:02 by ysirkich         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{ //z = z^2 + c z 
	...
	real = 0;
	imaginary = 0;
	c_real = fractal->min_real + (x / (double)WIDTH) * (fractal->max_real - fractal->min_real) + fractal->offset_x;
	c_imaginary = fractal->min_imag + (y / (double)HEIGHT) * (fractal->max_imag - fractal->min_imag) + fractal->offset_y;
	iterations = 0;
...
int	julia_pixel(t_fractol *fractal, int x, int y)//ill try rbg lets see lol
{
	...
	real = fractal->min_real + (x / (double)WIDTH) * (fractal->max_real - fractal->min_real) + fractal->offset_x;
	imaginary = fractal->min_imag + (y / (double)HEIGHT) * (fractal->max_imag - fractal->min_imag) + fractal->offset_y;
	iterations = 0;
	while (iterations < fractal->max_iterations)
	{
