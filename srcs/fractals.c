...
/*   Updated: 2024/11/13 10:04:13 by ysirkich         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{ //z = z^2 + c z 
	...
	real = 0;
	imaginary = 0;
	c_real = fractal->min_real + (x / (double)fractal->width) * (fractal->max_real - fractal->min_real);
	c_imaginary = fractal->min_imag + (y / (double)fractal->height) * (fractal->max_imag - fractal->min_imag);
	iterations = 0;
...
int	julia_pixel(t_fractol *fractal, int x, int y)//ill try rbg lets see lol
{
	...
	real = fractal->min_real + (x / (double)fractal->width) * (fractal->max_real - fractal->min_real);
	imaginary = fractal->min_imag + (y / (double)fractal->height) * (fractal->max_imag - fractal->min_imag);
	iterations = 0;
	while (iterations < fractal->max_iterations)
	{
